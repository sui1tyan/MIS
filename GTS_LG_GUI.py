...
- Generates/loads Ed25519 keypair (private_key.pem kept beside this tool).
- Writes license.json + public_key.pem into the selected *app* folder (next to your main app .py/.exe).
- Supports Admin PIN (4–16 chars; letters, digits, @#$%^&+=!?.-), optional hostname binding, optional expiry (days).

Dependencies:
  pip install customtkinter cryptography

Build note (PyInstaller one-file suggestion):
  pyinstaller --noconfirm --onefile --name "GTS_LG" GTS_LG_ui.py
"""
import os
import json
import hashlib
...
PIN_REGEX = re.compile(r"^[A-Za-z0-9@#$%^&+=!?.\\-]{4,16}$")  # same pattern as your CLI tool
...
def _log_path():
    return os.path.join(_app_dir(), \"gts_lg.log\")
...
def write_license(app_dir: str, pin: str, bind_hostname: bool, custom_hostname: str, expires_days: int):
    \"\"\"Create license.json + public_key.pem inside *app_dir* using/creating keypair next to this tool.\"\"\"
    ...
    if expires_days and int(expires_days) > 0:
        payload[\"expires_at\"] = (datetime.datetime.utcnow() + datetime.timedelta(days=int(expires_days))).isoformat(timespec=\"seconds\") + \"Z\"
...
class LGApp(ctk.CTk):
    def __init__(self):
        ...
        entry = ctk.CTkEntry(self, textvariable=self.app_dir, width=380); entry.grid(row=row, column=0, sticky=\"we\", **pad)
        ...
        cb = ctk.CTkCheckBox(self, text=\"Bind to hostname\", variable=self.bindhost, command=self._toggle_host)
        cb.grid(row=row, column=0, sticky=\"w\", **pad)
    ...
    def _toggle_host(self):
        # placeholder (could disable/enable hostname entry if needed)
        pass
...
def main_cli():
    ...
    ap.add_argument(\"--bind-hostname\", action=\"store_true\", help=\"Bind license to this machine's hostname (or --hostname if set)\")
    ap.add_argument(\"--hostname\", default=None, help=\"Custom hostname to bind to (overrides auto-detected)\")
